...
serve(async (req) => {
  ...
  try {
    ...
    let { formId, amount, plan, currency = 'USD' } = body;
    ...
    // Validação e correção de valores
    if (!amount || isNaN(amount) || amount <= 0) {
      console.log(`Valor inválido recebido: ${amount}, plano: ${plan}`);
      
      // Determinar valor padrão com base no plano
      if (plan === 'annual') {
        amount = 598.80; // Plano anual em BRL
        currency = 'BRL';
      } else if (plan === 'monthly') {
        amount = 89.90; // Plano mensal em BRL
        currency = 'BRL';
      } else {
        return new Response(
          JSON.stringify({ error: 'Valor de pagamento inválido' }),
          { 
            status: 400, 
            headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
          }
      ...
      console.log(`Usando valor padrão: ${amount} ${currency}`);
    }
